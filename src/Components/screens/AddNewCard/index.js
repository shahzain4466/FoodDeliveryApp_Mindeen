...
const AddNewCard = ({ navigation }) => {
  ...
  useEffect(() => {
    if (cardHolderName && cardDigit && cvc_cvv?.length > 2 && expiry_date && cardDigit?.length > 18) {
    ...
    <View style={styles.btmContainer}>
      ...
      <Button
        ...
        onContinue={() => {
          ...
          dispatch(AddPaymentAPI(cardNumber, cardHolderName, expiry_date, cvc_cvv, token))
        }}
      />
    </View>
  );
  ...
  const infoBtm = (
    <View style={styles.btmContainer}>
      <View style={styles.successTickIconContainer}>
        <Image
          source={Images.CVVinfo}
          style={styles.infomodalpic}
        />
      </View>
      <Txt
        style={[styles.infoBtmTitle, { marginVertical: theme.spacing.margin.m1 }]}>
        {Strings.cvvinfohead}<Text style={styles.subinfohead}>{Strings.cvvinfo}</Text>
      </Txt>
      <Button
        title={Strings.locBtnBtmOneTitle}
  ...
  return (
    <Frame
      ...
      snapPoints={showInfoBtm ? snapPointInfoBtm : null || showSecondBtm ? snapPointSaveBtm : snapPointConfirmBtm}
      bottomSheetContent={
        showInfoBtm ? infoBtm : null || showSecondBtm ? saveDetailBtm : closedBtm
      }
      // headerVariant={'blank'}
      ref={showInfoBtm ? informationBtm : null || showSecondBtm ? saveBtm : confirmBtm}
