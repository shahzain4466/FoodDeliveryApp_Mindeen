...
export default function MyAddresses({navigation, route}) {
  ...
  console.log('latandlong', markerLocation);
  ...
  useEffect(() => {
    if (getLocationLoading) {
      ...
      console.log('Data:-', JSON.stringify(getLocationData));
  ...
  // console.log('selectedItem ---> ', currentLocation?.latitude, currentLocation?.longitude);
          ...
          <MapView
            ...
            {currentLocation && (
              <Marker
                ...
                // console.log('onDrag', nativeEvent.coordinate);
  ...
  const footerComp = (
    <View style={styles.btnContainer}>
      <Button
        ...
        onPress={() => {
          navigation.navigate(Screens.OrderMap, {cartItem: cartItem});
        }}
      />
      <Button
        ...
        onPress={() => {
          if (selectedItem !== 'currentLocationComp') {
            const body = {
              orderInformation: {
            ...
            if (
              markerLocation.latitude !== 0 &&
              markerLocation.longitude !== 0
            ) {
              // Proceed with navigation using the selected location
              const body = {
                orderInformation: {
                  cartId: cartItem?.cartId,
                  deliveryAddress: {
