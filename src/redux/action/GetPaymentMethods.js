...
import {Started, Completed, Failed} from '../slices/GetPaymentMethodSlice'

export const GetPaymentMethods = token => async dispatch => {
  try {
    dispatch(Started());
    const endpoint = '/api/v1/payment-method/all';
    const method = 'GET';
    const data = null;
    const response = await callApi(endpoint, method, data, token);
    dispatch(Completed(response));
  } catch (error) {
    const errorDetail = handleErrors(error, '/api/v1/payment-method/all');
